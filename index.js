...
async function CHAMIBOT() {
	const conn = new WAConnection();
	conn.logger.level = 'warn';
	conn.version = [2, 2119, 6]

	conn.on('connecting', async () => {
		console.log(`${chalk.green.bold('T')}${chalk.blue.bold('CHAMI')}
${chalk.white.italic('CHAMIBOT String SESSION')}
${chalk.blue.italic('ℹ️  Connecting to Whatsapp... Please wait.')}`);
	});

	conn.on('open', async () => {
		console.log(
			chalk.green.bold('CHAMI-BOT QR Code: '),
			'CHAMIBOT;;;' +
